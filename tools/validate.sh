...
explain_output_checks() {
  ...
  [ ! -s "$explain_err" ] || return 1
  [ "$(cat "$version_out")" = 'git-hotspots 0.1.0-alpha.5' ] || return 1
  ...
  [ ! -s "$explain_err" ] || return 1
  [ "$(cat "$version_nongit")" = 'git-hotspots 0.1.0-alpha.5' ] || return 1
...
license_version_checks() {
  ...
  grep -q '0.1.0-alpha.5' README.md || return 1
  ...
  [ "$("$EXE" --version)" = 'git-hotspots 0.1.0-alpha.5' ] || return 1
...
allowed = (
    ...
    'tree-sitter-zig-relations',
    'tree-sitter-go-relations',
    'tree-sitter-python-relations',
    'tree-sitter-javascript-relations',
    'tree-sitter-lua-relations',
...
source_install_smoke() {
  ...
  [ "$($copy_exe --version)" = 'git-hotspots 0.1.0-alpha.5' ] || return 1
  ...
  printf 'source-install-copy %s version=0.1.0-alpha.5\n' "$summary" >> "$SMOKES"
}
...
printf 'validate: RUN relationship fixture realism matrix checks\n'
if relationship_fixture_realism_matrix_checks; then
  note_fallback "relationship fixture realism matrix: python3 compared matrix rows with admitted relationship goldens"
...
printf 'validate: RUN historical provider-state matrix checks\n'
if historical_provider_state_matrix_checks; then
  note_fallback "historical provider states: python3 compared matrix/audit rows with historical-symbol golden provider states"
  pass_rung "historical provider-state matrix checks"
else
  fail_rung "historical provider-state matrix checks" "historical provider-state matrix drifted from fixture goldens"
fi
printf 'validate: RUN historical fallback pressure checks\n'
if historical_fallback_pressure_checks; then
  note_fallback "historical fallback pressure: python3 compared fallback row and hunk pressure docs with historical-symbol golden counts"
