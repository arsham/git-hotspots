...
make_lua_symbols() {
  repo="$FIX/lua-symbols"
  rm -rf "$repo"
  mkdir -p "$repo/src"
  setup_repo "$repo"

  cat > "$repo/src/old-example.lua" <<'EOF'
...
return exports
EOF
  printf '' > "$repo/src/empty.lua"
  cat > "$repo/src/invalid_partial.lua" <<'EOF'
local function broken(
  return 1
EOF
  cat > "$repo/src/generated.lua" <<'EOF'
...
EOF
  cat > "$repo/src/dynamic_table_assignment.lua" <<'EOF'
local exports = {}
...
return exports
EOF
  cat > "$repo/src/metatable_heavy.lua" <<'EOF'
local Thing = {}
local mt = {
...
return Thing
EOF
  cat > "$repo/src/embedded_dsl.lua" <<'EOF'
...
EOF
  cat > "$repo/src/other.lua" <<'EOF'
local function other_only()
  return 1
end

return other_only
EOF
  cat > "$repo/src/target.lua" <<'EOF'
...
return target
EOF
  ln -s target.lua "$repo/src/link.lua"
  python3 - <<'PY' > "$repo/src/large.lua"
print('local function Large()')
print('  return 1')
print('end')
print('-- ' + 'x' * (1024 * 1024 + 1))
PY
  cat > "$repo/src/missing.lua" <<'EOF'
...
return missing
EOF
  commit_all "$repo" '2026-05-01T00:00:00+0000' 'initial lua symbol files'

  git -C "$repo" mv src/old-example.lua src/example.lua
  rm "$repo/src/missing.lua"
  commit_all "$repo" '2026-05-02T00:00:00+0000' 'expand lua symbol functions'
}
...
make_rust_symbols() {
  repo="$FIX/rust-symbols"
  rm -rf "$repo"
  mkdir -p "$repo/src"
  setup_repo "$repo"

  cat > "$repo/src/old-example.rs" <<'EOF'
...
EOF
  printf '' > "$repo/src/empty.rs"
  cat > "$repo/src/invalid_partial.rs" <<'EOF'
pub fn broken(
EOF
...
EOF
  cat > "$repo/src/unsupported.md" <<'EOF'
pub fn hidden() {}
EOF
  cat > "$repo/src/other.rs" <<'EOF'
pub fn other_only() {}
EOF
  cat > "$repo/src/target.rs" <<'EOF'
pub fn target() {}
EOF
  ln -s target.rs "$repo/src/link.rs"
  python3 - <<'PY' > "$repo/src/large.rs"
print('pub fn large() {}')
print('// ' + 'x' * (1024 * 1024 + 1))
PY
  cat > "$repo/src/missing.rs" <<'EOF'
pub fn missing() {}
EOF
  commit_all "$repo" '2026-05-01T00:00:00+0000' 'initial rust symbol files'

  git -C "$repo" mv src/old-example.rs src/example.rs
...
git clone -q --depth 1 "file://$FIX/rust-symbols" "$FIX/rust-symbols-shallow"
git clone -q "$FIX/rust-symbols" "$FIX/rust-symbols-partial"
git -C "$FIX/rust-symbols-partial" config remote.origin.promisor true
