...
pub const relation_provider_name = "tree-sitter-go-relations";
...
const relation_unsupported_caveats = [_][]const u8{
    "relation provider unsupported: only repo-relative .go files are parsed by the Go relation provider",
    "no parser diagnostics or source snippets exposed",
};
const relation_unavailable_caveats = [_][]const u8{
    "relation provider unavailable or current working-tree source was not available to the Go relation provider",
    "no parser diagnostics, source snippets, absolute paths, remotes, author identities, commit messages, or private repo names exposed",
};
...
test "extract relations emits bounded Go syntax candidates" {
    const source =
        \\// Code generated by local proof; DO NOT EDIT.
        \\//go:build linux
        \\package proof
    ...
    for (result.candidates) |relation| {
        ...
        try expectRelationCaveat(relation.caveats, "bounded Go syntax evidence: contains, import includes, direct identifier calls, selector-like syntax, unresolved identifiers, and unknown relation-like syntax");
    }
}

test "extract relations degrades and caps Go proof safely" {
    const source =
        \\package proof
        \\func one() {}
