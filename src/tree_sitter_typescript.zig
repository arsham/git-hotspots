...
fn extractPathWithContract(allocator: std.mem.Allocator, io: std.Io, repo_root: []const u8, repo_relative_path: []const u8, contract: Contract) !Extraction {
    ...
    const link_stat = std.Io.Dir.statFile(.cwd(), io, full_path, .{ .follow_symlinks = false }) catch return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
    if (link_stat.kind != .file) return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});

    const file = std.Io.Dir.openFileAbsolute(io, full_path, .{}) catch return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
    defer file.close(io);

    const stat = file.stat(io) catch return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
    if (stat.kind != .file or stat.size > max_file_bytes) return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});

    const source = allocator.alloc(u8, @intCast(stat.size)) catch return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
    defer allocator.free(source);
    const bytes_read = file.readPositionalAll(io, source, 0) catch return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
    if (bytes_read != source.len) return extraction(allocator, contract, repo_relative_path, .unavailable, .unknown, .unknown, &unavailable_caveats, &.{});
