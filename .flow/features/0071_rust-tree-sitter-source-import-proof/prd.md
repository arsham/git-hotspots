# Rust Tree-sitter source import proof

## Summary

Import a pinned, narrow `tree-sitter-rust` source set into the repository as a
source-import and provenance proof only. The feature resolves the Rust admission
blockers from Feature 0063 that can be proven at source-import time: immutable
...
compatibility, source-size impact, offline validation, `.rs` admission evidence,
and no-provider output stability.

This feature must not add Rust parser build integration, runtime provider
...
remote enrichment, Cargo or package analysis, module resolution, macro
expansion, type checking, LSP, custom user queries, or background provider
execution.

## Problem

Feature 0063 recorded `tree-sitter-rust` as a plausible future grammar but left
implementation blocked on exact immutable source identity, license and notice
verification, generated parser and scanner provenance, BOM selection, `.rs`
extension admission proof, source-size evidence, core compatibility, offline
validation, and no-provider output stability. Rust runtime support should not
proceed until the source import itself is reviewable and local-first.

## Outcome

A pinned `tree-sitter-rust` source set is vendored under
`third_party/tree-sitter-rust/<revision>/` with provenance, imported-file
manifest, license and notice handling, `.rs` extension admission evidence, and
source-import evidence. The repository still has no Rust runtime support after
...
- Select an immutable `tree-sitter-rust` revision: tag plus full commit, or
  full commit when no suitable tag exists.
- Import a narrow source set under `third_party/tree-sitter-rust/<revision>/`.
...
Rust grammar. Expected paths may include generated parser files, scanner files,
...
Rust grammar. If a scanner is present, it must be included in the BOM only when
...
custom user queries, Cargo workspace metadata, crate graph artefacts,
dependency graph artefacts, macro-expansion artefacts, and provider
implementation unless a file is explicitly justified in the BOM.

### R6 Tree-sitter core compatibility

The import must prove that the selected Rust grammar language version is
...
### R7 Rust path admission evidence

The import must record that future Rust runtime work is limited to requested
inspect paths ending in `.rs`. It must not imply `Cargo.toml` parsing, Cargo
workspace discovery, crate graph analysis, dependency graph inference, macro
expansion, type checking, module resolution, LSP integration, or repo-wide
scanning.
...
- bytes added under the vendored Rust grammar path;
...
compatibility, `.rs` extension admission, source-size evidence, validation
commands, local/offline boundary, and explicit statement that Rust runtime
support is not implemented yet.

### R10 Existing behaviour preservation

Existing proof targets and product validation must continue to pass. Existing
Zig, Go, Python, JavaScript, Lua, TypeScript, and TSX symbol outputs and
no-provider table, JSON, Markdown, and inspect outputs must remain byte-stable
unless a separate feature explicitly changes them.

## Acceptance criteria

- A narrow, pinned `tree-sitter-rust` source set exists under a revisioned
  vendored path.
- The import has deterministic provenance, BOM, license, notice, parser,
  scanner, core-compatibility, source-size, and `.rs` path evidence.
- Public evidence exists at `docs/tree-sitter-rust-source-import.md`.
- No Rust build proof, query fixtures, extraction, runtime provider, inspect
...
- If the upstream identity differs from Feature 0063, record the reason and do
...
- If source-size or validation impact is unexpectedly high, escalate before
  closing.
- If no safe local sibling repository has Rust files, record a bounded
