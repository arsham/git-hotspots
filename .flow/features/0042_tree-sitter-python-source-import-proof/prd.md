# PRD: Tree-sitter Python source import proof

## Summary

Import a pinned, narrow `tree-sitter-python` source set into the repository as a
source-import and provenance proof only. The feature resolves the admission
blockers from Feature 0041 that can be proven at source-import time: immutable
...
compatibility, source-size impact, offline validation, and no-provider output
stability.

This feature must not add Python parser build integration, Python runtime
...
enrichment, or background provider execution.

## Problem

Feature 0041 recorded `tree-sitter-python` as a plausible future grammar but
left it in `conditionalpython` state. The remaining blockers include exact
immutable source identity, license and notice verification, generated parser and
scanner provenance, BOM selection, source-size evidence, core compatibility,
offline validation, and no-provider output stability. Python runtime support
should not proceed until the source import itself is reviewable and local-first.

## Outcome

A pinned `tree-sitter-python` source set is vendored under
`third_party/tree-sitter-python/<revision>/` with provenance, imported-file
manifest, license/notice handling, and source-import evidence. The repository
still has no Python runtime support after this feature; it only has reviewed
local source inputs for a later Python build/extraction proof.

## Requirements

### R1 Source import scope

- Select an immutable `tree-sitter-python` revision: tag plus full commit, or
  full commit when no suitable tag exists.
...
The import must identify generated parser and scanner files, expected to
include `src/parser.c` and any `src/scanner.c`, `src/scanner.cc`, or equivalent
scanner files if present. It must do one of:
...
- required parser headers or binding metadata when needed by the generated C
  sources;
- helper headers required by the scanner, such as local Tree-sitter allocation
  or array headers when the selected revision requires them;
...
The import must prove that the selected Python grammar language version is
...
If the selected Python grammar requires a newer Tree-sitter core, this feature
must stop or record a blocker. A Tree-sitter core update is out of scope.
...
Python provider output is out of scope and must not be produced by this
feature.

### R10 Public evidence and future sequencing

The evidence document must state that Python runtime support is still not
implemented. It must identify the next likely follow-up as Python offline
build/extraction proof or record blockers if import gates failed.

It must explicitly defer:

- Python parser compile/link proof;
- Python extraction proof;
- Python symbol query fixtures;
...
- Python package, virtual environment, `pyproject.toml`, dependency graph,
  import resolution, notebook, workspace, or repo-wide scanning behaviour.

## Acceptance criteria

- `third_party/tree-sitter-python/<revision>/` exists with a narrow imported
  source set.
- The vendored Python component includes unchanged upstream license/notice text.
- The vendored Python component includes `PROVENANCE.md` and
  `IMPORTED_FILES.tsv`.
- `THIRD_PARTY_NOTICES.md` is updated with public, repo-relative Python
  attribution.
- A source-import evidence document records immutable source identity, generated
  parser/scanner provenance, Tree-sitter core compatibility, source-size
  measurements, offline validation, and no-provider output stability.
- No Python build, runtime provider, provider registry, CLI, report schema,
  scoring, cache, test fixture, CI, release, or package behaviour is added.
- The feature stops as blocked rather than closing if immutable revision,
  ...
  narrowness, or no-provider stability cannot be proven.
...
- If generated parser or scanner files cannot be reproduced or verified, stop as
  blocked.
- If the Python grammar uses an external scanner, include it only if it is
  required by the generated parser and provenance/license gates are satisfied.
- If the Python grammar requires helper headers for scanner compilation, include
  only the required local headers and justify each one in the BOM.
- If the Python grammar requires a newer Tree-sitter core than the current
...
- changed paths are limited to `third_party/tree-sitter-python/<revision>/`,
...
- Python parser language version is compatible with Tree-sitter core language
  range `13..15`;
...
- docs and evidence use only repo-relative paths and public upstream
  identifiers;
