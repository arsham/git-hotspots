...
Feature 0071 is expected to import pinned Rust Tree-sitter sources with
...
The build graph can compile and link the vendored Rust parser and scanner
sources offline in a local proof target, with deterministic evidence and no
Rust runtime provider, inspect output, default report, scoring, cache, Cargo,
crate graph, package, module, macro expansion, type checking, LSP, network,
telemetry, upload, or remote-enrichment behaviour.

## Requirements

### R1 Build proof scope

1. Add a narrow build proof for the vendored Rust parser and scanner sources
   imported by Feature 0071.
...
4. Do not add Rust runtime provider registration, CLI behaviour, report schema
   fields, default output changes, scoring changes, cache changes, query
   fixtures, symbol extraction behaviour, Cargo/package/module analysis, macro
   expansion, type checking, LSP, CI, release, network, telemetry, upload, or
   remote enrichment.

### R2 Build-system shape

1. Prefer a proof target that mirrors the JavaScript, TypeScript, and Lua
   build-proof precedents while keeping Rust independently identifiable.
2. The proof must compile parser and scanner objects for Rust using
...
5. The proof must not require Rust, Cargo, package managers, generated grammar
   regeneration, global Tree-sitter CLI, or network access.

### R3 Evidence

1. Add or update a public evidence document for the Rust build proof.
2. Evidence must identify the exact vendored source revision and source paths
   used.
3. Evidence must record Rust parser/scanner compile/link commands or validation
   targets.
...
1. The repository has a deterministic offline build proof for the selected Rust
   parser and scanner sources.
2. The proof uses only vendored, repository-local inputs and does not require
   Rust, Cargo, package managers, grammar regeneration, network, telemetry,
   upload, or remote enrichment.
3. The proof does not expose Rust runtime provider behaviour or change default
   reports, inspect output, report schemas, scoring, cache, CLI, CI, release,
   package, LSP, Cargo, crate graph, package, module, macro expansion, type
   checking, or dependency behaviour.
...
1. If Feature 0071 has not recorded an exact vendored Rust source path, stop and
   return to dispatch after that dependency closes.
2. If Rust scanner compilation needs a wrapper or C++ treatment, document the
   difference and keep the proof explicit.
3. If the grammar requires a newer Tree-sitter core, stop for a separate
   core-update feature.
4. If the build proof needs generated files not imported by Feature 0071, stop
   and reshape instead of silently expanding the BOM.
5. If local sibling smoke has no safe Rust file, record a bounded privacy-safe
...
zig build test
zig build
Rust parser/scanner compile/link proof
