# Tree-sitter Python offline build proof

This is the Feature 0043 execution evidence for the non-product Python parser
build proof. It compiles the already-vendored Tree-sitter core and
`tree-sitter-python` parser sources through the Zig build graph, runs a tiny
in-memory Python parse, and stops before any Python provider runtime, query, or
...
- `build.zig`: explicit `tree-sitter-python-build-proof` build step and helper
  functions for the proof-only Python grammar inputs.
- `tests/tree_sitter_python_build_proof.zig`: tiny non-product proof
  executable.
- `.github/workflows/ci.yml`: CI invocation for the proof step.
- `docs/tree-sitter-python-offline-build-proof.md`: this evidence record.
...
network enrichment, background analysis, package manifests, submodules,
generated parser regeneration, a system Tree-sitter package, a C
package-discovery dependency, a global Tree-sitter CLI dependency, or Python
symbol extraction behavior.
...
test ! -e build.zig.zon
test ! -e .gitmodules
```

Observed result: no output, and both absence checks passed.
...
- `third_party/tree-sitter-python/v0.25.0/src`

The proof executable parses this in-memory source:
...
It asserts that the root node is `module`, that no parse error is present, and
that the single named child is `function_definition`. It is not installed by the
normal product build and is not wired into the CLI or provider registry.

## Scanner handling

The Python generated parser declares external scanner entry points and
`EXTERNAL_TOKEN_COUNT 12`, so the proof compiles `src/scanner.c` alongside
`src/parser.c`. The scanner uses only the vendored local helper headers already
imported by Feature 0042 under `src/tree_sitter/`. No scanner source is
regenerated, fetched, or supplied by a system package.

## CI membership

CI now runs the proof explicitly after the existing validation and Tree-sitter
Zig/Go proof steps:
...
and close-out evidence still require the offline Python parser compile/link
smoke.
...
- Command: `zig build --cache-dir <tmp>/cache --global-cache-dir <tmp>/global
  tree-sitter-python-build-proof`
- Exit status: `0`.
- Elapsed wall-clock time: `5470` ms.
- Proof executable size in the isolated Zig cache: `16379384` bytes.
- Sanitised output observation: no stdout or stderr were emitted.

Cached repo-local proof command:

- Command: `zig build tree-sitter-python-build-proof`.
- Exit status: `0`.
- Sanitised output observation: no stdout or stderr were emitted.

Result: no duplicate-symbol, missing-symbol, scanner-link, compile, or link
failure was observed for the Tree-sitter core `lib.c` single-translation-unit
path linked with the vendored Python parser and scanner. If this later needs an
individual-file compile set, parser generation, package-manager fetch, system
package, network access, Python provider semantics, or symbol extraction,
execution must stop for planning instead of broadening the proof locally.
...
Python parser proof step is separate and does not affect scoring, report
rendering, inspect semantics, current-symbol enrichment, or cache behavior.
...
| `zig build` | `0` | Normal product build completed; `zig-out/bin` contained only `git-hotspots`. |
| `zig build validate` | `0` | Default validation rungs passed, including this-repo smoke, deterministic fixture checks, runtime dependency scan, and prohibited claim scan. |
...
| `zig build tree-sitter-python-build-proof` | `0` | Python parser and scanner proof compiled, linked, and ran with no stdout/stderr. |
...
identity, raw sibling output, private person identities, source snippets,
remote URLs, and private history text. Validation reported local-only behavior:
no network transfer, external reporting, CI service dependency, default Python
provider runtime, cache requirement, packaging, release automation, or
background analysis.
...
commercial strategy language, unsupported behavioral claims, or people-metric
claims.
